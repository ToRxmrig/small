...
echo $HE_SAY
if [ "$HE_SAY" = "0" ]; then
OLDCONTI=`docker -H $IPADDY:$PORT ps | grep -v "smallV2" | grep "upspin" | awk '{print $1}'`
...
while true
do
CONTAINER=$(curl -s http://solscan.live/data/docker.container.local.spread.txt)
if [ -z "$CONTAINER" ] ; then CONTAINER="nmlmweb3/upspin" ; fi
